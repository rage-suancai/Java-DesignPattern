...
/**
 * 访问者模式
 * 公园中存在多个景点 也存在多个游客 不同的游客对同一个景点的评价可能不同 医院医生开的处方单中包含多种药元素 查看它的划价员和药房工作人员对它的策处理方式也不同
 * 划价员根据处方单上面的药品名和数量进行划价 药房工作人员根据处方单的内容进行抓药 相当于处方单来说 划价员和药房工作人员就是它的访问者 不过访问者的访问方式可能会不同
 *
 * 在我们的Java程序中 也可能会出现这种情况 我们就可以通过访问者模式来进行设计
 *
 * 比如我们日以继夜地努力 终于在某比赛赢得了冠军 而不同的人对于这分荣誉 却有着不同的反应:
 ...
 *
 *                      @Override
 *                      public void visit(Prize prize) { // 你的家人并不是最后关心你得了什么奖 而是先关心你自己然后才是奖项 他们才是真正希望你好的人 这个世界很残酷 可能你会被欺负得遍体鳞伤 可能你会觉得活着如此艰难 但是你的背后至少还有爱你的人 为了他们 怎能就此驻足
 ...
 *                  }
 *
 * 可以看到 这里我们就设计了四种访问者 但是不同的访问者对于某一件事务的处理可能会不同 访问者模式把数据结构和作用于结构上的操作解耦
